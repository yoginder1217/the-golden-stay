...
const VALUE_ICONS = [ShieldCheck, Users, Heart];

const About = () => {
  const { c, cJSON } = useSiteContent();
  const stats  = cJSON('about.stats');
  const values = cJSON('about.values');
    ...
    <div className="bg-gray-50 overflow-hidden">
      ...
      <div className="bg-golden shadow-xl relative z-20 -mt-10 mx-4 md:mx-auto max-w-6xl rounded-xl overflow-hidden">
        <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-charcoal/10">
          {(stats.length ? stats : [{label:'Happy Families',value:'500+'},{label:'Luxury Apartments',value:'50+'},{label:'Cities Covered',value:'5'},{label:'Star Rating',value:'4.9'}]).map((stat, idx) => (
            <motion.div
              key={idx}
              initial={{ opacity: 0 }}
      ...
      <div className="relative py-24">
        ...
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-16 items-center mb-32 relative z-10">
          <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="relative">
            <div className="absolute inset-0 border-2 border-golden transform translate-x-4 translate-y-4 rounded-2xl" />
            <img
          ...
          <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
            <span className="text-golden font-bold tracking-widest uppercase mb-2 block">{c('about.story.eyebrow')}</span>
            <h2 className="text-4xl font-bold text-charcoal mb-6 font-serif">{c('about.story.title')}</h2>
            <p className="text-gray-600 mb-6 leading-relaxed text-lg">{c('about.story.p1')}</p>
            <p className="text-gray-600 leading-relaxed text-lg mb-8">{c('about.story.p2')}</p>
            <div className="flex gap-4 items-center bg-white p-4 rounded-xl shadow-sm border border-golden/20 inline-block">
              ...
              <p className="font-bold text-charcoal italic">
                {c('about.award.text')} <br />
                <span className="text-xs font-normal text-gray-500">{c('about.award.source')}</span>
              </p>
            </div>
          </motion.div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          <div className="text-center mb-16">
            ...
            <div className="h-1 w-24 bg-golden mx-auto rounded-full" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {(values.length ? values : [{title:'Uncompromised Safety',desc:''},{title:'Family Centric',desc:''},{title:'Warm Hospitality',desc:''}]).map((item, idx) => {
